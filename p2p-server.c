...
/*
	NOTE: This is a snapshot of a working STUN/matchmake server.
	Development continuing in mmsrv.c
*/

#define INET_ADDRSTRLEN 16
#define debug(fmt, ...) __debug (__func__, __LINE__, fmt, ## __VA_ARGS__)
#define assert(EXP) if (!(EXP)) { fprintf (stdout, "%s:%d> assertion failed\n", __func__, __LINE__); *(int *) 0 = 0; }
...
static u32
generate_id_from_peer (ENetPeer *peer)
{
	char addr[255];

    htop (peer->address.host, peer->address.port, addr, sizeof (addr));
...
get_enet_peer_by_id (ENetHost *host, u32 id)
{
	...
	debug ("peers:%p, peerCount:%d, connectedPeers:%d\n",
           host->peers, host->peerCount, host->connectedPeers);

	for (peer = host->peers;
         peer < &host->peers[host->peerCount];
         ++peer)
    {
		debug ("  peer:%p id:%d state:%d\n", peer, peer->incomingPeerID, peer->state);
		if (peer->state == ENET_PEER_STATE_CONNECTED)
		{
			u32 peer_id = generate_id_from_peer (peer);
			if (peer_id == id)
			{
				return peer;
			}
...
//			dump_enet_peers (server);
            ...
            g__running = true;
            printf ("Starting NAT punch-through server [%d]\n", port);
            ...
            while (g__running)
            {
				...
				if (t > 500)
				{
					...
					if (g__dump_state) debug ("clients: %u\n", client_count);		
					...
					for (u32 i = 0; i < client_count; i++)
					{
						...
						htop (c->pub.host, c->pub.port, pu, sizeof (pu));
						htop (c->priv.host, c->priv.port, pr, sizeof (pr));
					...
					if (g__dump_state) g__dump_state = false;
					...
					if (num_lfp >= 2)
					{
						...
						ENetPacket *to_a = enet_packet_create (
                            (void *) &p,
                            sizeof (struct join_packet),
						...
						ENetPacket *to_b = enet_packet_create (
                            (void *) &p,
                            sizeof (struct join_packet),
                            ENET_PACKET_FLAG_RELIABLE);
                ...
                while (enet_host_check_events (server, &event) > 0)
                {
                    switch (event.type)
                    {
                        case ENET_EVENT_TYPE_CONNECT:
                        {
                			...
                			htop (event.peer->address.host, event.peer->address.port, addr, sizeof (addr));
                			u32 id = generate_id_from_peer (event.peer);

                			debug ("client [%u] connected from [%s]\n", id, addr);
...
//                			event.peer->data = ptr_from_u32 (id);
							debug ("enet peer %p state: %d\n", event.peer, event.peer->state);
                        ...
                        case ENET_EVENT_TYPE_RECEIVE:
                        {
							...
							htop (priv->host, priv->port, ip, sizeof (ip));
